...
# time.sleep can be removed to run the program faster.
...
class Blockchain(object):
    ...
    def show_blocks_info(self, start, end):
        for i in range(start, end + 1):
            pprint(self.blockChain[i])
...
def main():
    ...
    blockchain3.get_blocks_from_nodes()
    blockchain4.get_blocks_from_nodes()
    blockchain5.get_blocks_from_nodes()
    ...
    while True:
        ...
        client.pos()
        if len(blockchain5.blockChain) > end_index:
            break
    
    blockchain5.show_blocks_info(start_index, end_index)
